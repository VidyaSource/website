...
- An attacker who manages to commit a bad `AGENTS.md` defeats the drift half outright. Run the audit in CI, where the checkout starts
  clean and the build itself becomes the only thing that can dirty it, and let the ASCII half plus a human reviewer summoned with a
  `CODEOWNERS` [file](https://docs.github.com/en/repositories/managing-your-repositorys-settings-and-features/customizing-your-repository/about-code-owners) cover the rest.
...
- **Set `ignore-scripts=true` in `.npmrc`**, or run `npm ci --ignore-scripts`. That genuinely stops npm lifecycle code. Python
  ...
  arbitrary code on your machine. However, it fails outright for any package that publishes no wheel, and a wheel can still ship a
  `.pth` file that runs at interpreter startup. Gradle has no equivalent switch, so pin your plugin jars with
  `gradle --write-verification-metadata sha256 help` and commit the result. Just understand that this only covers an install-time attack
   and leaves NVIDIA's build-time attack completely untouched.
...
- **Mount `AGENTS.md` read-only in your [Dev Container](https://containers.dev).** This is the coolest because it is the only layer here that prevents
  ...
  its mode. Any read-only `chmod 444` protection flips right back to writable `644` before an attack appends whatever it likes. POSIX
...
"mounts": ["source=${localWorkspaceFolder}/AGENTS.md,target=${containerWorkspaceFolder}/AGENTS.md,type=bind,readonly"]
```

That mount needs one entry per `AGENTS.md`. That might seem annoying, but I don't mind because every entry forces you to be deliberate
about which files you actually sanction. Some things are important enough to demand your attention.
...
and `docs/CONFIG.md`, wide open. Add those paths to the list if that keeps you up at night. Give `AGENTS.md` the same suspicion you would
...
art in crafting a strong `AGENTS.md`, to learn how to complement it with Agent Skills, and to protect it from attack, then we can turn our opinions into
a blueprint for building great software and delivering the best experiences for our customers.
